...
- ``GUARD_SYSTEM_PROMPT``：字段级严格 + 来源合法 + 不跨实体传染的系统指令
...
GUARD_SYSTEM_PROMPT = (
    ...
    "2. 推理级允许：对概念解释、差异总结、共性提炼、建议性结论，允许基于上下文做合理推断，"
    "但必须在句末注明依据：（依据：xxx.md）或（依据：现有资料）。\n"
    "3. 来源合法：答案中引用的文件名 / 手册名 / 表名必须出现在"
    "『可引用的来源列表』里；不确定来源时写「（依据：现有资料）」而非编造文件名。\n"
...
def build_comparison_user_prompt(
    *,
    ...
    return (
        ...
        "- 表格最后一列为「来源」，列出支撑该行的文件名"
        "（只能是『可引用的来源列表』里出现过的文件）。\n"
        "- 严禁把 A 实体的字段填到 B 实体的单元格。\n\n"
        "### 3. 选型建议（可选）\n"
        "- 仅当上下文里出现明确的应用场景、产品定位、适用规模等描述时才写。\n"
        "- 句式：「根据 <来源文件名> 中描述的 <场景/定位>，建议选 <实体>」。\n"
        "- 上下文里没有选型相关描述时，直接省略这一节，不要编造场景。\n\n"
        "### 4. 资料缺口\n"
...
def build_generic_user_prompt(
    *,
    ...
    """通用（非对比）问题的 user prompt。与历史通用模板兼容。"""
    sources = allowed_sources or []
    sources_line = ", ".join(sources) if sources else "（无命中来源）"
    return (
        "请根据『检索上下文』回答用户问题，使用 Markdown 格式。\n"
        f"问题类型：{query_mode}。\n"
        "- 命中结构化事实时，优先给出精确字段值、表名和行号；\n"
