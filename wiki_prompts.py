...
GUARD_SYSTEM_PROMPT = (
    ...
    "如果上下文来自表格、名单、日程等逐行记录，必须保持行级原子性："
    "同一行里的时间、主讲、内容、地点、名称等字段只能和该行其他字段配对，"
    "绝不把不同行的字段重新拼接成一条新记录。\n"
    ...
    "6. 术语解释优先：若检索上下文包含 GLOSSARY / 术语解释，"
    "这些解释只代表当前知识库内的约定含义；回答相关问题时必须优先采用，"
    "不要用其他行业或其他实体的同名术语含义覆盖。\n"
    "7. 上下文为空或严重不足时：坦率写「当前知识库中缺少直接证据」，不要编造。\n"
)
...
def build_generic_user_prompt(
    *,
    ...
    return (
        ...
        "文件名、手册名等来源信息；\n"
        "- 若上下文包含表格、名单、排期、通讯录等逐行记录，必须按原记录逐行整理；"
        "同一行的主讲、内容、时间、地点、名称等字段不能跨行拼接；\n"
