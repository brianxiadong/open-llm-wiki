...
**首页 / 仓库列表**：顶部不再展示大面积用户 hero，而是改为紧凑的操作条。当前登录用户访问自己的列表页时，顶部固定提供两个入口：「添加共享知识库」与「新建知识库」；前者通过弹窗输入访问码完成加入，后者直接进入创建页。下方列表按「我的知识库」与「共享给我的知识库」分组，知识库卡片统一展示图标、名称、owner、公开/角色徽标、摘要和来源/Wiki 统计；共享库卡片额外突出共享来源，并把“退出共享”作为次级操作固定在卡片底部。
