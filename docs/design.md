...
**查询界面**：上方输入框，下方显示回答（markdown 渲染）。回答中的 Wiki 引用可点击跳转。owner 可见「保存为 Wiki 页面」按钮。输入框右侧的会话相关操作使用带文字的可见按钮（如「历史记录」「清空对话」），避免只靠图标导致功能不可发现；这些工具按钮与发送按钮不仅共享统一控件高度，还会沿输入条纵向拉伸，尽量贴合整块输入容器，减少知识库页底部操作区的视觉错位；公开仓库访客不显示会话栏。
