...
GET  /repos/join/{access_code}          → 通过邀请链接直接加入共享知识库（未登录则先跳登录）
...
**知识库设置页**（`templates/repo/settings.html`）：由「基本信息」「共享访问码」「共享成员」「Wiki Schema」「导入 Wiki（ZIP）」「README」「删除知识库」七个独立表单区块组成；owner 可在这里生成 viewer/editor 访问码、复制邀请链接、停用访问码以及移除共享成员。新生成共享邀请后，页面会先自动尝试把邀请链接复制到剪贴板；若浏览器拦截自动复制，会在成功提示卡片内保留“复制邀请链接”按钮和明确说明，方便用户一键重试后直接发给同事，而不再额外展示完整邀请地址。共享访问码列表只展示访问码文本，并保留“复制邀请链接”按钮，不再提供“复制访问码”按钮，也不再把完整邀请地址直接渲染在表格中。每个表单都显式携带 `action` 隐藏字段，避免未启用 CSRF 模板变量时提交丢失操作类型。
...
仓库额外提供一个可共享的 OpenClaw skill 样例：`openclaw-skills/openllm-kb-search/`。该 skill 负责首次收集并保存 `ollw_...` token，在用户显式指定知识库时先通过 `/api/v1/repos` 解析仓库，再调用 `/api/v1/search`；未指定仓库时直接走服务端自动路由。
...
│   └── deploy.sh          ← 部署脚本
├── openclaw-skills/
│   └── openllm-kb-search/ ← 面向 OpenClaw 的共享 skill（token 保存 + repo 路由查询）
├── packaging/
...
**部署脚本**（`scripts/deploy.sh`）：在项目根本地 `.env` 中配置 `DEPLOY_HOST`、`DEPLOY_PORT`、`DEPLOY_USER`、`DEPLOY_PASSWORD`（可选 `DEPLOY_PATH`，默认 `/opt/open-llm-wiki`），执行 `./scripts/deploy.sh` 时会自动 `source` 该 `.env`。这些凭据仅保存在本地工作区，**不提交 Git**。脚本用 `sshpass` 上传 tarball，并将仓库内的 `deploy/llmwiki.service` 安装到 `/etc/systemd/system/llmwiki.service`；解压代码后会先执行 `.venv/bin/pip install -r requirements.txt` 同步生产依赖，再执行迁移、`daemon-reload`、重启 `llmwiki` 服务并校验 unit 已加载 `/opt/open-llm-wiki/.env`。服务器上的业务 `.env` 与 `data/` 不会被部署覆盖。
