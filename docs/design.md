...
**知识库设置页**（`templates/repo/settings.html`）：由「基本信息」「共享访问码」「共享成员」「Wiki Schema」「导入 Wiki（ZIP）」「README」「删除知识库」七个独立表单区块组成；owner 可在这里生成 viewer/editor 访问码、复制邀请链接、停用访问码以及移除共享成员。新生成共享邀请后，页面会先自动尝试把邀请链接复制到剪贴板；若浏览器拦截自动复制，会在成功提示卡片内保留“复制邀请链接”按钮和明确说明，方便用户一键重试后直接发给同事，而不再额外展示完整邀请地址。共享访问码列表也只保留复制按钮，不再把完整邀请地址直接渲染在表格中。每个表单都显式携带 `action` 隐藏字段，避免未启用 CSRF 模板变量时提交丢失操作类型。
