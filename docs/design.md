...
**来源证据页**（`templates/source/list.html`、`templates/source/view.html`）：公开仓库允许访客查看来源列表和来源 Markdown 预览，用于承接 evidence 链接；上传、下载原始文件、删除、重命名和摄入按钮仅 owner 可见。文档管理页的上传流程包含明确的“已选择文件”确认态，URL 导入区使用可展开面板承载二级操作；批量删除和批量摄入按钮默认禁用，只有勾选文件后才进入可点击状态，避免动态态和静态态样式割裂。
