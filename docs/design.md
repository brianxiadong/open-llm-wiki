...
**知识库设置页**（`templates/repo/settings.html`）：由「基本信息」「共享访问码」「共享成员」「Wiki Schema」「导入 Wiki（ZIP）」「README」「删除知识库」七个独立表单区块组成；owner 可在这里生成 viewer/editor 访问码、复制邀请链接、停用访问码以及移除共享成员。每个表单都显式携带 `action` 隐藏字段，避免未启用 CSRF 模板变量时提交丢失操作类型。
