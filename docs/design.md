...
**仓库面板**：左侧是来源文件与 Wiki 页面的导航区，右侧是聊天式查询区域。标题下方提供紧凑型的「知识库操作」条，不再把高频能力隐藏在三点菜单中；功能按「文档与导入 / 检索与探索 / 维护与治理 / 导出与设置」分组展示，每个入口直接显示名称，既保留可发现性，又尽量减少对聊天首屏的占用。公开仓库的非 owner 访客不显示会话栏，查询退化为无状态对话。
