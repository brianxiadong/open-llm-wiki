...
**首页 / 仓库列表**：以“知识库首页”形态承载列表页。顶部使用左侧介绍 hero + 右侧“快速开始”操作卡片的双栏布局：hero 展示当前用户、产品说明和概览统计卡（我的知识库 / 共享给我 / 当前视图），操作卡片集中提供“输入访问码加入共享知识库”和“新建知识库”两个入口，并附带简短使用提示。下方列表按「我的知识库」与「共享给我的知识库」分组，知识库卡片统一展示图标、名称、owner、公开/角色徽标、摘要和来源/Wiki 统计；共享库卡片额外突出共享来源，并把“退出共享”作为次级操作固定在卡片底部。

**新建知识库页**（`templates/repo/new.html`）：采用左侧说明 hero + 右侧创建表单卡片的双栏布局。说明区概述后续上传、Wiki、问答能力；表单区集中填写名称、URL 标识、Schema 模板和描述，并通过固定的返回入口回到知识库列表，整体视觉与知识库首页保持统一。
...
**来源证据页**（`templates/source/list.html`、`templates/source/view.html`）：公开仓库允许访客查看来源列表和来源 Markdown 预览，用于承接 evidence 链接；上传、删除、重命名和摄入按钮对 owner/editor 可见，下载原始文件对已登录且有仓库访问权的成员可见。文档管理页采用顶部说明 hero + 上传/任务卡片布局，下方再展示带筛选和批量操作的文件列表；上传流程包含明确的“已选择文件”确认态，批量删除和批量摄入按钮默认禁用，只有勾选文件后才进入可点击状态。内部系统前端不再展示“从 URL 导入网页”入口。
...
**知识库设置页**（`templates/repo/settings.html`）：采用左侧设置概览/锚点导航 + 右侧设置卡片栈的双栏布局，包含「基本信息」「共享访问码」「共享成员」「Wiki Schema」「导入 Wiki（ZIP）」「README」「删除知识库」七个独立表单区块；owner 可在这里生成 viewer/editor 访问码、停用访问码以及移除共享成员。每个表单都显式携带 `action` 隐藏字段，避免未启用 CSRF 模板变量时提交丢失操作类型。
