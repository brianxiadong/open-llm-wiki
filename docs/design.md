...
| 文档解析       | 调用已部署的 MinerU 服务（[http://172.36.237.175:8000）](http://172.36.237.175:8000）) |
...
经 MinerU 解析的上传（PDF/DOCX/PPTX/图片等）在服务端先写入 `temp/<uuid>.<ext>`，再以纯 ASCII 路径调用 MinerU，避免部分环境下 multipart 使用中文原始文件名导致解析失败；生成的 `.md` 与 `raw/originals/` 下留存仍使用用户可见原始文件名。
...
```
GET /{username}/{repo}/query/stream?q=<question>&reasoning_mode=standard|deep|react
        │
        ▼
    event: progress  (message="正在检索相关页面…", percent=10；深度模式下另有「拆解检索要点」「检索步骤 i/n」等进度)
        │
        ▼ (双通道检索；深度/极致在生成前另有「评审检索结果」「补充检索」类进度)
    event: progress  (message="正在读取页面内容…", percent=40)
        │
        ▼
    ...
    event: answer_chunk  (chunk="...")  × N
        │
        ▼
    event: done  (…, reasoning_mode, sub_questions, react_trace, retrieval_critique)
        │
...
POST /{username}/{repo}/query            → 提交查询（含多轮会话上下文；可选 `reasoning_mode`: standard|deep|react）
GET  /{username}/{repo}/query/stream     → SSE 流式查询（EventSource；Query 参数 `reasoning_mode`，公开库可访客访问）
...
GET  /api/system/llm-status                  → 顶栏大模型状态探测（轮询 OpenAI 兼容接口，供前端绿灯/红灯展示）
GET  /api/system/dependencies-status         → 顶栏组件状态探测（聚合 Embedding / MinerU / Qdrant，供前端绿灯/红灯展示）
```
...
**仓库面板**：左侧是来源文件与 Wiki 页面的导航区，右侧是聊天式查询区域。聊天输入条左侧提供「标准模式 / 深度推理 / 极致推理」下拉框（`#chat-reasoning-mode`）：标准模式为单次选页与单次向量/事实召回；深度推理为 LLM 拆分子问题后对每步分别检索并合并证据后再回答；极致推理为 ReAct 式多轮「思考—检索—观察」后再回答（详见 §4.2 SSE 流式路径）。标题下方提供紧凑型的「知识库操作」条，不再把高频能力隐藏在三点菜单中；功能按「文档与导入 / 检索与探索 / 维护与治理 / 导出与设置」分组展示。共享成员采用三层权限：`owner` 拥有完整设置/删除/共享管理能力，`editor` 可上传文档、编辑 Wiki、保存问答结果与管理任务，`viewer` 仅浏览、搜索、问答。若历史数据导致 `page_count` / `source_count` 与磁盘实际内容不一致，面板渲染时会自动回填并纠正计数。侧边栏卡片标题中的名称与计数使用同一不可换行标题组展示，避免出现 `Wiki (30)` 被拆成两行；来源区与 Wiki 区则分别使用带高度上限的独立滚动容器，文档或页面很多时优先在卡片内部滚动，而不是把整个侧边栏拖成长滚动条。公开仓库的未登录访客不显示会话栏，查询退化为无状态对话；已登录的共享成员则拥有自己的会话历史。
...
**时区**：元数据与 ORM 默认值仍以 **UTC** 写入数据库；面向用户的 HTML 模板、部分 JSON（如会话列表时间、`/api/v1` 中仓库与 Token 的 `updated_at`/`expires_at` 等）、查询追溯 JSONL 的 `ts`、Wiki 摄入 log 与 frontmatter 中的「今天」日期、顶栏依赖探测的 `checked_at` 等，统一按环境变量 `**APP_TIMEZONE`**（默认 `Asia/Shanghai`）换算展示。systemd 单元 `deploy/llmwiki.service` 设置 `TZ=Asia/Shanghai`，便于进程级日志时间一致。
...
RAG_FACT_KEYWORD_TOP_K=100
RAG_FACT_KEYWORD_MAX_RECORDS=50000
RAG_FACT_SEARCH_TEXT_CHARS=2000
...
# 可选：覆盖页脚与 GET /health 的 revision（默认见 deploy/revision.txt 或 git 短 SHA）
# APP_REVISION=custom-label
```
...
**Dense**：fact embedding 前缀写入 `[source | 表=<sheet> | 行=<idx>]`，提升字段定位语义。
**关键词 + 精确（默认开启，`RAG_ENABLE_FACT_KEYWORD`）**：`QdrantService.scroll_all_facts(repo_id)`（桌面端 `ConfidentialQdrantService` 读 `fact_map`）拉全量 payload，在内存中为每行构造 `fact_search_text`（`source_file`、`sheet`、`row_index`、各 `字段名=值`、原 `fact_text` 等，长度上限 `RAG_FACT_SEARCH_TEXT_CHARS`），经 `KeywordIndex` 做 BM25；另用规则对查询与字段名/值做轻量精确加分（`exact_score`，无 LLM）。三路结果按 RRF 融合（exact 项在 `fused_score` 上叠加 `exact_score`），去重键为 `record_id`。单库 fact 行数超过 `RAG_FACT_KEYWORD_MAX_RECORDS` 时跳过关键词通道并打日志，避免超大表拖慢内存。
...
5. **目录生成**：长页面自动生成 TOC
6. **行内星号列表归一**：`utils.normalize_inline_bullet_markdown` 在 `render_markdown` 前将「如下： * 项A * 项B」类单行输出拆成标准列表（列表前空行 + 行首 `*`，且不误改 fenced code），避免聊天/追溯页只显示一串 `*` 正文。

### 8.5 MinerU 文档解析集成

通过 HTTP 调用已部署的 MinerU 服务（[http://172.36.237.175:8000）完成文档解析。](http://172.36.237.175:8000）完成文档解析。)
...
- 响应体结构：
  ...
  ```
- OpenClaw 等外部前端接入时，若接口已返回 `answer`，应优先直接展示该 Markdown 正文，不再对表格、名单、日程、参数表做二次总结式改写；否则容易把 `facts[].fields` 中原本属于不同记录行的字段重新拼接，导致“主讲/内容/时间”错位。
- 仓库内提供 `openclaw-skills/openllm-kb-search/` 与 `openclaw-skills/kb/` 两套 OpenClaw skill：前者负责 token 校验、显式/自动路由与 `/api/v1/search` 调用，后者把用户显式输入的 `/kb ...` 固定收敛到“快捷直返”模式。该模式要求在接口已经返回 `answer` 后，最终回复只保留极短的命中/路由说明并直接返回 `answer` 原文，避免再次生成长文导致延迟放大和表格字段错位。
...
│   ├── pack-internal-kb-assistant-zip.sh ← 打包公开「使用小助手」Wiki ZIP（导入用）
│   └── deploy.sh          ← 部署脚本
├── seed/
│   └── internal-kb-assistant/ ← 面向最终用户的使用手册 Markdown（wiki/），不含技术实现说明
├── packaging/
...
- MinerU 服务（[http://172.36.237.175:8000）——文档解析](http://172.36.237.175:8000）——文档解析)
- MySQL（172.36.164.85:3306）——元数据存储
- Qdrant（[http://172.36.164.85:6333）——向量检索](http://172.36.164.85:6333）——向量检索)
- Embedding 模型（[http://172.36.237.245:11434/v1）——文本向量化](http://172.36.237.245:11434/v1）——文本向量化)
...

{
  ...
  "revision": "a1b2c3d · 2026-04-22 16:00:00 +0800"
}
```

任何一个服务不可达时 status 为 `degraded`（而非 `error`，因为有降级策略）。`revision` 与全站页脚版本号同源，用于确认当前运行代码是否为目标构建。
